...
class AdditionLayer(nn.Module):
    def forward(self, t_1: Tensor, t_2: Tensor):
        return t_1 + t_2
...
class PadLayer(nn.Module):
    def forward(
        self, tensor: Tensor, padding: Tuple = (1, 1, 1, 1), value: float = None
    ):
        if value is None:
...
class SoftMaxLayer(nn.Module):
    def forward(self, tensor: Tensor, dim: int | None = 1):
...
class CatLayer(nn.Module):
    ...
    def forward(
        self, tensor_list: Tuple[Tensor, ...] | List[Tensor], dim: int | None = 1
    ):
